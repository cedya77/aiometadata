...
export function StremThruIntegration({ isOpen, onClose }: StremThruIntegrationProps) {
  ...
  const getCatalogKey = (catalog: StremThruCatalog) => {
    ...
    try {
      setConfig(prev => {
        ...
        selectedCatalogs.forEach(catalogKey => {
          ...
          // Generate unique catalog ID: stremthru.{manifestId}.{type}.{catalogId}
          const manifestId = manifest.id.replace(/[^a-zA-Z0-9]/g, '_');
          const uniqueCatalogId = `stremthru.${manifestId}.${catalog.type}.${catalog.id.replace(/[^a-zA-Z0-9]/g, '_')}`;
