...
const createGeminiDispatcher = () => {
    ...
    if (geminiProxy) {
        try {
            return new ProxyAgent({
                uri: new URL(geminiProxy).toString(),
                allowH2: false
            });
        } catch (error) {
    ...
    if (globalProxy) {
        try {
            return new ProxyAgent({
                uri: new URL(globalProxy).toString(),
                allowH2: false
            });
        } catch (error) {
...
class ConfigApi {
    ...
    validateCatalogCount(config) {
        ...
        if (enabledCount <= maxCatalogs) return {
            valid: true
        };
        return {
            valid: false,
                            ...
                            if (oldCatalog) {
                                if (newCatalog.sort !== oldCatalog.sort || newCatalog.order !== oldCatalog.order) {
                                    ...
                                    logger.debug(`MDBList catalog ${newCatalog.id} sort/order changed:`, {
                                        old: {
                                            sort: oldCatalog.sort,
                                            order: oldCatalog.order
                                        },
                                        new: {
                                            sort: newCatalog.sort,
                                            order: newCatalog.order
                                        }
                                    });
                                }
                            }
    ...
    async migrateFromLocalStorage(req, res) {
        try {
            ...
            if (!localStorageData) {
                return res.status(400).json({
                    error: 'localStorage data is required'
                });
            }
    ...
    async loadConfigFromDatabase(userUUID) {
        try {
            ...
            const cachedConfig = await configCache.getOrLoad(userUUID, async () => {
                ...
                // --- TVDB DVD Fallback Sanitization ---
                if (config.tvdbFallbackIds) {
                    if (typeof config.tvdbFallbackIds === 'string') {
                        config.tvdbFallbackIds = config.tvdbFallbackIds.split(',').map(id => id.trim()).filter(Boolean);
                    } else if (Array.isArray(config.tvdbFallbackIds)) {
                        config.tvdbFallbackIds = config.tvdbFallbackIds.map(id => String(id).trim()).filter(Boolean);
                    }
                } else {
    ...
    validateAndNormalizeTestApiKeys(apiKeys) {
        if (!apiKeys || typeof apiKeys !== 'object' || Array.isArray(apiKeys)) {
            return {
                error: "apiKeys must be a plain object."
            };
        }
        ...
        for (const [key, value] of Object.entries(apiKeys)) {
            if (!TESTABLE_API_KEY_FIELDS.has(key)) {
                return {
                    error: `Unsupported apiKeys field '${key}'.`
                };
            }

            if (typeof value !== 'string') {
                return {
                    error: `API key '${key}' must be a string.`
                };
            }
            ...
            if (trimmedValue.length > maxKeyLength) {
                return {
                    error: `API key '${key}' exceeds max length (${maxKeyLength}).`
                };
            }
        ...
        if (Object.keys(normalizedApiKeys).length === 0) {
            return {
                error: "At least one non-empty API key is required."
            };
        }
    ...
    async validateApiKeys(apiKeys) {
        ...
        const testFunctions = {
            ...
            publicmetadb: async (key) => {
                const {
                    validateKey
                } = require('../utils/publicmetadbUtils');
    ...
    async testApiKeys(req, res) {
        try {
            ...
            const {
                details,
                summary
            } = await this.validateApiKeys(validation.apiKeys);
            res.json({
                success: true,
                details,
                summary
