...
async function buildParameters(type: string, language: string, page: number, id: string, genre: string, genreList: any[], config: UserConfig): Promise<any> {
  ...
  if (config.strictRegionFiltering && language) {
    ...
    if (regionCode) {
      ...
      // Ensure provider lookups use the correct region (for Streaming catalogs)
      ...
      // For TV shows, we must specify monetization types for watch_region to work effectively as a general filter
      if (type === 'series') {
